    ...
    <div className="bg-white rounded-xl overflow-hidden mb-6 border border-gray-100 shadow-xs">
      ...
      <div className="flex items-center justify-between px-6 py-4 border-b border-gray-50">
        <div className="flex items-center gap-6">
          <h2 className="text-xl font-black text-orange-500 italic">Flash Sale</h2>
          <div className="flex items-center gap-2 text-sm font-bold">
            <span className="text-gray-500">Ending in</span>
            <div className="flex gap-1">
               <span className="bg-orange-500 text-white w-8 h-8 flex items-center justify-center rounded-md">{formatTime(hours)}</span>
               <span className="text-orange-500 flex items-center">:</span>
               <span className="bg-orange-500 text-white w-8 h-8 flex items-center justify-center rounded-md">{formatTime(minutes)}</span>
               <span className="text-orange-500 flex items-center">:</span>
               <span className="bg-orange-500 text-white w-8 h-8 flex items-center justify-center rounded-md">{formatTime(seconds)}</span>
            </div>
          </div>
        </div>
        <Link href="#" className="text-sm font-bold text-orange-500 border border-orange-500 px-4 py-1.5 rounded-sm hover:bg-orange-50 transition-colors uppercase tracking-tight">
          Shop More
        </Link>
      </div>
        ...
        {flashSaleProducts.map((product) => (
          <Link href="#" key={product.id} className="min-w-[160px] max-w-[160px] snap-start group relative">
            <div className="relative w-full h-40 bg-gray-50 rounded-lg overflow-hidden mb-2">
              ...
              />
              <div className="absolute top-0 right-0 bg-yellow-400 text-orange-900 text-[10px] font-black px-2 py-1 rounded-bl-lg z-10">
                -{product.discount}%
              </div>
            </div>
            
            <h3 className="text-sm text-gray-800 line-clamp-2 mb-1 group-hover:text-orange-500 h-10">{product.title}</h3>
            
            <div className="flex flex-col">
               <span className="text-lg font-black text-orange-500">${product.price.toFixed(2)}</span>
               <span className="text-xs text-gray-400 line-through">${product.originalPrice.toFixed(2)}</span>
            </div>

            <div className="mt-2 w-full bg-orange-100 rounded-full h-1.5 overflow-hidden">
               <div className="bg-orange-500 h-full rounded-full" style={{ width: `${product.sold}%` }}></div>
            </div>
