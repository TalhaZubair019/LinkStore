      ...
      <div className="container mx-auto px-4 py-12">
        <div className="flex gap-4 overflow-x-auto pb-4">
          {[1, 2, 3, 4, 5, 6, 7, 8].map((i) => (
            <div key={i} className="min-w-[120px] h-[140px] bg-slate-100 dark:bg-slate-800 animate-pulse rounded-2xl" />
          ))}
        </div>
      </div>
    );
    ...
    <section className="bg-white dark:bg-slate-950 py-16 transition-colors duration-300 overflow-hidden">
      <div className="container mx-auto px-4 max-w-7xl">
        <div className="flex items-center justify-between mb-10">
          ...
          >
            Show All <ChevronRight size={16} className="group-hover:translate-x-1 transition-transform" />
          </Link>
        </div>
          ...
          {categories.map((category, idx) => (
            ...
            >
              ...
              >
                ...
                <span className="text-[10px] font-black uppercase tracking-widest text-slate-500 dark:text-slate-400 text-center line-clamp-1 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">
                  {category.title}
                </span>
              </Link>
            </motion.div>
