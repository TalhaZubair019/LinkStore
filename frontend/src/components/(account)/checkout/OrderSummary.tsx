...
export default function OrderSummary({
  ...
  const groupedCartItems = cartItems.reduce((acc: any, item: any) => {
    ...
    if (!acc[storeName]) {
      acc[storeName] = { items: [], storeSubtotal: 0 };
    }
    acc[storeName].items.push(item);
    acc[storeName].storeSubtotal += item.price * (item.quantity || 1);
    return acc;
  }, {});
  return (
    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg p-6 lg:p-8 shadow-sm sticky top-40 transition-colors">
      ...
      <div className="space-y-8 mb-6">
        {Object.entries(groupedCartItems).map(([storeName, group]: [string, any]) => (
          <div key={storeName} className="space-y-4">
            <h4 className="flex items-center gap-2 text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest pb-2 border-b border-slate-100 dark:border-slate-800">
              <span className="w-2 h-2 rounded-full bg-purple-500"></span> Sold by {storeName}
            </h4>
                ...
                <div key={item.id} className="flex gap-4 items-start">
                  ...
                  <div className="flex-1">
                    <div className="flex justify-between items-start">
                      <h4 className="font-bold text-slate-700 dark:text-slate-200 text-sm transition-colors pr-2">
                        {item.name}
                      </h4>
                      <span className="text-sm font-medium text-slate-600 dark:text-slate-400 transition-colors shrink-0">
            ...
            <div className="text-right text-[11px] font-bold text-slate-400 dark:text-slate-500 mt-2">
              Subtotal: ${group.storeSubtotal.toFixed(2)}
            </div>
          </div>
        ))}
      </div>
