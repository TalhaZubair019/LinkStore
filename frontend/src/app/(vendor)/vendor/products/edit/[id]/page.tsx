...
export default function EditProductPage() {
  ...
  const handleGenerateDescription = async () => {
    ...
    try {
      const res = await fetch("/api/vendor/ai-description", {
        method: "POST",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        router.push(`/vendor/dashboard?tab=products&page=${fromPage}`);
      } else {
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen font-sans flex items-center justify-center bg-slate-50 dark:bg-[#0d0f14] transition-colors duration-300">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              <div className="bg-white dark:bg-white/3 rounded-3xl shadow-[0_20px_50px_rgba(0,0,0,0.02)] dark:shadow-2xl border border-slate-100 dark:border-white/5 p-8 space-y-4 relative overflow-hidden group">
                ...
                <Link
                  href={`/vendor/dashboard?tab=products&page=${fromPage}`}
